...
export function Post({ post, postBody }) {
  ...
  return (
    <div className="h-full transition-all duration-300 pb-7 bg-white pt-3 w-[35rem] rounded">
      ...
      <div onClick={() => setBodyExpanded((expanded) => !expanded)}  className="flex hover:bg-gray-50 rounded-md cursor-pointer justify-between  transition-all duration-200 pl-[2%] mt-2   text-center py-2 mx-[6%] gap-3">
        <h1 className="text-[25px] textshadow2">{post.title}</h1>
        <img
          src="/down (1).png"
          className={`w-[1rem] h-[1rem] my-auto translate-x-[-1rem]  translate-y-[5px] transition-all duration-200 ${bodyExpanded? "rotate-[180deg]": ""} `}
