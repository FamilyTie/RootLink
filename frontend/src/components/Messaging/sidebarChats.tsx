...
const SidebarChats = ({ onSelectChatroom, refresh, userid, chatRoomId }) => {
  ...
  return (
    <div className="bg-white      text-black h-[93vh] overflow-hidden p-4  border-black">
      <input type="text" placeholder="Start a chat" className="w-[95%] placeholder:font-semibold  border p-2 bg-slate-100 rounded mb-4" />
      <div className="flex justify-center mb-10 overflow-scroll">
        {
          initialUsers.map((user) => {
            return <div onClick={() => newChat(user.id)}  className="cursor-pointer">
              <img src={user.img}  alt={user.username} className="w-16 h-16 shadow p-[2px] bg-[#A0D9FF] rounded-full mr-3" />
              <p className="font-semibold text-gray-500 text-center r">{user.fullName.split(' ')[0]}</p>
            </div>
          })
        }
      </div>
      ...
      <ul>
        {chatrooms.length === 0 && <p className="text-[20px] m-auto text-gray-400 font-semibold text-center">No Recent Chats</p>}
        {chatrooms.map((chatroom) => (
          <li
            key={chatroom.id}

            className={`flex gap-3 ${chatRoomId === chatroom.id? "bg-gray-200" :" bg-white hover:hover:bg-gray-100 "} items-center p-2 mb-2 rounded-lg   transition-all duration-200 cursor-pointer`}
