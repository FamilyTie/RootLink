...
function SideBar() {
  ...
  return (
    <div>
      <div
        className={`  z-[501] w-[14%]    h-full fixed bg-white   backdrop-blur-md   overflow-hidden  flex flex-col  pt-[5rem]   transition-all duration-200  border border-blue-500`}
      >
        {images.map((image) => (
          <div
            key={image.id}
            ...
            className={` pl-6 py-3   ${
              clicked === image.id
                ? " border-l-[5px] bg-gray-50  border-[#074979]"
