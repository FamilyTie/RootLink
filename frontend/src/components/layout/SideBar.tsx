...
function SideBar() {
    const images = [
        {id: 1, src: ['/home (1).png', '/home (2).png'], text: 'Home'},
  ...
  return (
    <div className={` w-[14%]   h-full fixed bg-white   backdrop-blur-md   overflow-hidden  flex flex-col  pt-[5rem]   transition-all duration-200  `}>
       {images.map((image) => (
        <div key={image.id} onClick={() => setClicked(image.id)} className={` pl-6 py-3 pt-5 ${clicked === image.id? " border-l-[5px] bg-gray-50  border-[#074979]" : ""} cursor-pointer transition-all duration-300  gap-5 flex`}>
          <img
